...
bool Email::AlfabetoMinusculoOuDigito(char c) const {
  return (isalpha(c) and islower(c)) or isdigit(c); 
}

void Email::ValidarEmail(const std::string& email) {
  if (email.empty()) { throw std::invalid_argument("O email digitado esta vazio!"); }
